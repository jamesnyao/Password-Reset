...
foreach my $reqid (grep(/^req_[0-9]+$/, $cgireq->param)) {
  ...
  my $newpass = krb_set_encrypted_password($id, $ARGV[0], $auth);
  ...
  foreach my $entry ($lgdata->entries) {
    if($entry->get_value('gidNumber') < 100) {  # Access Control Groups
...
#  $taddrs{$id} = $em;
  ...
  if($ltdata->entry(0)->get_value('homeDirectory') eq 'disabled') {
    ...
    my $newpass = krb_encrypted_password($target, $reqnum, $auth);
...
#    ldap_update_entry($ltdata->entry(0)) || goto endreq;
  ...
  my $newpass = krb_set_password($reqnum, $target, $auth);
